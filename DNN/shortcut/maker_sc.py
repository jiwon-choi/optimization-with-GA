...
def fileMaker(gene):
    ...
    f.write("L0_sc = tf.placeholder('float', [None, 784])\n")
    ...
    if(actF == 'relu'):
        f.write("L1 = tf.nn.relu(tf.add(tf.matmul(L0_sc, W1), B1))\n")
        f.write("L1_sc = tf.add(L0_sc, L1)\n")
        for i in range(2, layer+1):
            f.write("L" + str(i) + " = tf.nn.relu(tf.add(tf.matmul(L" + str(i-1) + ", W" + str(i) + "), B" + str(i) + "))\n")
            f.write("L" + str(i) + "_sc = tf.add(L" + str(i-1) + "_sc, L" + str(i) + ")\n")
    elif(actF == 'sigmoid'):
    ...
    f.write("hypothesis = tf.add(tf.matmul(L" + str(layer) + "_sc, W" + str(layer+1) + "), B" + str(layer+1) + ")\n\n")
    ...
    f.write("            sess.run(optimizer, feed_dict = {L0_sc: batch_xs, Y:batch_ys})\n\n")
    f.write("            avg_cost += sess.run(cost, feed_dict = {L0_sc:batch_xs, Y:batch_ys})/total_batch\n\n")
    ...
    f.write("    print(\"Accuracy=\", accuracy.eval({L0_sc:mnist.test.images, Y:mnist.test.labels}), \"normal_dnn\")")
